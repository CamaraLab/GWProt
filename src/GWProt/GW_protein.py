...
class GW_protein:
    ...
    def run_GW_from_cells(
        ...
        return GW_identity_init(cell_1, cell_2, transport_plan= transport_plan)
        
    @controller.wrap(limits=1, user_api=controller.info()[-1]['user_api'])
    @staticmethod       
    def run_GW(P1 :'GW_protein',
        P2: 'GW_protein',
        transport_plan:bool = False) -> Union[float, tuple[float, np.array]]:
        """
    ...
    def validate(self) -> bool:
        """
        Checks if a ``GW_protein`` object passes basic tests.
    ...
    @controller.wrap(limits=1, user_api=controller.info()[-1]['user_api'])
    @staticmethod
    def run_FGW_data_lists(p1: 'GW_protein', p2:'GW_protein', data1 :list[float] , data2 : list[float] , alpha:float = 1, transport_plan = False) -> Union[float, tuple[float, np.array]]:
        """
    ...
    @controller.wrap(limits=1, user_api=controller.info()[-1]['user_api'])
    @staticmethod
    def run_FGW_diff_mat(p1: 'GW_protein', p2:'GW_protein', diff_mat: np.array ,alpha:float = 1, transport_plan: bool = False) -> Union[float, tuple[float, np.array]]:
        """
        This calculates the fused Gromov-Wasserstein distance between two proteins. 
        :param p1: The first protein
        :param p2: The second protein
        :param diff_mat: A user-inputted matrix of the differences in the feature space between the residues of the two proteins. Of shape ``(len(p1),len(p2))``. \
        ``diff_mat[i,j]`` is the difference in features between the ith residue of ``p1`` and the jth residue of ``p2``.
        :param alpha: The trade-off parameter in [0,1] between fused term and geometric term. 
        A higher value of ``alpha`` means more geometric weight, ``alpha = 1`` is equivalent to regular GW.
        :param transport_plan: Whether to return the transport plan
        :return: Returns the FGW distance and transport plan if ``transport_plan``
