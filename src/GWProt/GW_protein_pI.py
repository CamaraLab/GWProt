...
# this is a subclass of GW_protein that has the isoelectric point features
...
class GW_protein_pI(GW_protein):
    ...
    def GW_protein(self):
        """
        Casts to a GW_protein object
        """
        return GW_protein(coords = self.coords, ipdm = self.ipdm, seq = self.seq, scaled_flag = self.scaled_flag,name = self.name, 
    ...
    def scale_ipdm(self,
        ...
        if inplace:
            ...
            return GW_protein_pI(seq = self.seq, pI_list = self.pI_list, ipdm = m, coords = None, name = self.name+'_scaled', scaled_flag = True, distribution = self.distribution)
    ...
    def downsample_by_indices(self, 
        ...
        return GW_protein_pI(seq = new_seq, pI_list = pI_list, ipdm = ipdm, coords = coords, name = self.name+'_downsampled', scaled_flag = self.scaled_flag, distribution = new_distribution)
    ...
    @staticmethod
    def FGW_lgd(prot1: 'GW_protein_pI', prot2:'GW_protein_pI',  alpha: float, T : np.array):
    #now with FGW lgd
        n1= len(prot1)
        n2 = len(prot2)
