...
------------------------
 The *unbalanced Gromov-Wasserstein distance* between 
proteins :math:`X` and :math:`Y` is defined as:

.. math::
   UGW(X, Y) = \min_{T} \frac{1}{2} \left( \sum_{i, j, k, l} \left[  \cdot |d_X(x_i, x_j) - d_Y(y_k, y_l)|^2   \cdot T_{i, k} T_{j, l} \right] + \rho \cdot KL^\otimes(\pi_1(T) | \mu_X) + \rho \cdot KL^\otimes(\pi_2(T) | \mu_Y)  \right)^{1/2}
...
.. math::
   UGW(X, Y) = \min_{T} \frac{1}{2} \left( \sum_{i, j, k, l} \left[  \cdot |d_X(x_i, x_j) - d_Y(y_k, y_l)|^2    \cdot T_{i, k} T_{j, l} \right] + (1 - \alpha) \cdot \sum_{i,j} \delta(x_i, y_k) \cdot T_{i, k} + \rho \cdot KL^\otimes(\pi_1(T) | \mu_X) + \rho \cdot KL^\otimes(\pi_2(T) | \mu_Y)  + \right)^{1/2}
